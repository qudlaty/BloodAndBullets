  ...
  showRoomList = [
    ...
    },
    {
      ...
      content: (
        <>
          {" "}
          🛢️ ={">"} <EmojiMapper emoji="🛢️" />, 🕷️ ={">"} <EmojiMapper emoji="🕷️" />{" "}
        </>
      ),
    },
    { name: "HpBar 66/100", content: <HpBar current={66} max={100} color="green"></HpBar> },
    { name: "HpBar 80/100 white", content: <HpBar current={80} max={100} color="white"></HpBar> },

    { name: "HpBar 0/100", content: <HpBar current={0} max={100} color="green"></HpBar> },
    { name: "HpBar -20/100", content: <HpBar current={-20} max={100} color="green"></HpBar> },
    { name: "HpBar 150/100", content: <HpBar current={150} max={100} color="green"></HpBar> },
    ...
    { name: "LinearDisplay with no label", content: <LinearDisplay current={13} max={20}></LinearDisplay> },
  ];

  render() {
    const theListToRender = this.showRoomList.map((item, index) =>
      item.name ? (
        <li key={index}>
    ...
    return (
      <div className="show-room">
        <div>
          ...
          <p>
            Be aware than this exposed some problems with the components working in isolation from the context they were
            designed for. A great opportunity for improvements.
          </p>
        </div>
        <ul>{theListToRender}</ul>
      </div>
    );
  }
}
